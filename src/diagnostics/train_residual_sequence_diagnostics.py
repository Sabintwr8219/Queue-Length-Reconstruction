"""
Standalone residual sequence diagnostic.

Purpose
-------
Train only GRU/LSTM residual models for:

1) CCT + ML
2) CCT + ML + CV

The supervised target is the residual from the cumulative-count theory queue:
    target_residual_from_baseline_ft = q_gt_ft - q_baseline_fixed_ft

The feasibility losses are applied to the implied queue length:
    q_pred_ft = q_baseline_fixed_ft + residual_pred_ft

This file is diagnostic-only. Once the visual behavior is acceptable, the same
loss logic can be moved back into the production residual training scripts.
"""
...
OUT_DIR = PROJECT_ROOT / "output" / "intermediate_csv" / "residual_sequence_diagnostics"
...
# Constraint-aware GRU/LSTM training. The model predicts a residual, but the
# physical penalties are applied to the implied queue length in feet.
...
LAMBDA_ZERO_QUEUE_MATCH = 0.20
LAMBDA_RESIDUAL_DQ = 0.08
LAMBDA_RESIDUAL_D2Q = 0.22
...
PHYSICS_ML_CATEGORICAL_FEATURES = [
    "phase_state",
]

PHYSICS_ML_CV_NUMERIC_FEATURES = [
    "phase_elapsed_sec",
    "A_count",
    "D_count",
    "V_count",
...
PHYSICS_ML_CV_CATEGORICAL_FEATURES = [
    "phase_state",
]
...
MODEL_FAMILIES = [
    ModelFamily(
        family_name="physics_ml",
        prediction_prefix="physics_ml",
        numeric_features=PHYSICS_ML_NUMERIC_FEATURES,
        categorical_features=PHYSICS_ML_CATEGORICAL_FEATURES,
        uses_cv_features=False,
        deduplicate_run_time=True,
        sequence_group_cols=["run_id"],
    ),
    ModelFamily(
        family_name="physics_ml_cv",
        prediction_prefix="physics_ml_cv",
        numeric_features=PHYSICS_ML_CV_NUMERIC_FEATURES,
        categorical_features=PHYSICS_ML_CV_CATEGORICAL_FEATURES,
        uses_cv_features=True,
        deduplicate_run_time=False,
        sequence_group_cols=["run_id", "cv_rate_pct"],
    ),
]
...
def load_feature_table() -> pd.DataFrame:
    ...
    for col in sorted(set(PHYSICS_ML_NUMERIC_FEATURES + PHYSICS_ML_CV_NUMERIC_FEATURES)):
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    df = df.dropna(subset=["run_id", "cv_rate_pct", "time_sec", TARGET_COL, GT_COL, BASELINE_Q_COL]).copy()
...
if TORCH_AVAILABLE:
    ...
    class SequenceWindowDataset(Dataset):
        ...
        def __getitem__(self, idx):
            x, y, baseline_q, q_true = self.windows[idx]
            return (
                torch.tensor(x, dtype=torch.float32),
                torch.tensor(y, dtype=torch.float32),
                torch.tensor(baseline_q, dtype=torch.float32),
...
def build_windows_for_groups(
    ...
    for _, g in work.groupby(group_cols, sort=True):
        ...
        baseline_group = baseline_q_ft[idx]
        q_true_group = q_true_ft[idx]

        if len(idx) <= sequence_len:
            windows.append((X_group, y_group, baseline_group, q_true_group))
            continue
        ...
        for s in starts:
            e = s + sequence_len
            windows.append((X_group[s:e], y_group[s:e], baseline_group[s:e], q_true_group[s:e]))
...
def physical_queue_constraint_loss(
    ...
    q_pred_ft = baseline_q_ft + residual_pred_ft
...
def residual_correction_smoothness_loss(
    ...
    The physical curve already carries the main queue rise/dissipation pattern.
    The learned residual should correct systematic bias, not create rapid local
    reversals in the reconstructed queue.
    """
    residual_pred_ft = pred_scaled * target_scale + target_mean
...
def train_rnn_model(
    ...
    baseline_q_all = df[BASELINE_Q_COL].to_numpy(dtype=float)
    q_true_all = df[GT_COL].to_numpy(dtype=float)

    train_windows = build_windows_for_groups(
        df=df,
        feature_matrix=X_all_scaled,
        y_scaled=y_all_scaled,
        baseline_q_ft=baseline_q_all,
        q_true_ft=q_true_all,
    ...
    for epoch in range(1, NN_EPOCHS + 1):
        ...
        for xb, yb, baseline_b, q_true_b in train_loader:
            xb = xb.to(DEVICE)
            yb = yb.to(DEVICE)
            baseline_b = baseline_b.to(DEVICE)
            q_true_b = q_true_b.to(DEVICE)

            optimizer.zero_grad(set_to_none=True)
            pred = model(xb)
            mse_loss = loss_fn(pred, yb)
            constraint_loss = physical_queue_constraint_loss(pred, baseline_b, target_mean, target_scale)
            dynamics_loss = supervised_queue_dynamics_loss(pred, baseline_b, q_true_b, target_mean, target_scale)
            shape_loss = supervised_queue_shape_loss(pred, baseline_b, q_true_b, target_mean, target_scale)
        ...
        if val_loader is not None:
            ...
            with torch.no_grad():
                for xb, yb, baseline_b, q_true_b in val_loader:
                    xb = xb.to(DEVICE)
                    yb = yb.to(DEVICE)
                    baseline_b = baseline_b.to(DEVICE)
                    q_true_b = q_true_b.to(DEVICE)
...
def train_one_family(
    ...
    if TRAIN_GRU:
        if not TORCH_AVAILABLE:
            ...
            residual_col = f"residual_pred_gru_{family.prediction_prefix}_ft"
            q_col = f"q_pred_gru_{family.prediction_prefix}_ft"
            df[residual_col] = predict_rnn_by_group(
                model=gru_model,
                df=df,
                feature_matrix_unscaled=X_all,
            ...
            df[q_col] = df[BASELINE_Q_COL].to_numpy(dtype=float) + df[residual_col].to_numpy(dtype=float)
            if CLIP_QUEUE_PREDICTIONS_TO_NONNEGATIVE:
                df[q_col] = np.maximum(df[q_col].to_numpy(dtype=float), 0.0)
            pred_cols_created.append(q_col)
    ...
    if TRAIN_LSTM:
        if not TORCH_AVAILABLE:
            ...
            residual_col = f"residual_pred_lstm_{family.prediction_prefix}_ft"
...
def compute_metrics_by_run_rate(pred: pd.DataFrame) -> pd.DataFrame:
    ...
    for (run_id, rate), g in pred.groupby(["run_id", "cv_rate_pct"], sort=True):
        ...
        for model_name, q_col in q_cols.items():
            ...
            if "_physics_ml_cv" in model_name:
                family = "physics_ml_cv"
            elif "_physics_ml" in model_name:
                family = "physics_ml"
            else:
                family = "unknown"
...
def get_diagnostic_prediction_columns(pred: pd.DataFrame) -> list[str]:
    cols = []
    for col in [
        "q_pred_gru_physics_ml_ft",
        "q_pred_lstm_physics_ml_ft",
...
def diagnostic_label(col: str) -> str:
    mapping = {
        ...
        "q_baseline_fixed_ft": "Cumulative Count Theory",
        "q_pred_gru_physics_ml_ft": "CCT + ML GRU",
        "q_pred_lstm_physics_ml_ft": "CCT + ML LSTM",
        "q_pred_gru_physics_ml_cv_ft": "CCT + ML + CV GRU",
        "q_pred_lstm_physics_ml_cv_ft": "CCT + ML + CV LSTM",
    }
    return mapping.get(col, col)


def save_residual_full_run_plot(pred: pd.DataFrame, run_id: int, rate: int) -> None:
    df = pred[
    ...
    if not pred_cols:
        print("[WARN] No GRU/LSTM residual prediction columns for full-run diagnostic plot.")
        return
    ...
    if BASELINE_Q_COL in df.columns:
        ax.plot(
            df["time_sec"],
            pd.to_numeric(df[BASELINE_Q_COL], errors="coerce"),
            color="0.45",
            linewidth=1.35,
            linestyle="--",
            label="Cumulative Count Theory",
        )

    colors = {
        "q_pred_gru_physics_ml_ft": "tab:red",
        "q_pred_lstm_physics_ml_ft": "tab:orange",
        "q_pred_gru_physics_ml_cv_ft": "tab:purple",
        "q_pred_lstm_physics_ml_cv_ft": "tab:blue",
    }
    ...
    ax.set_title(f"Residual sequence diagnostic | Run {run_id:03d} | CV {rate}%")
    ...
    out_path = FIG_DIR / f"residual_sequence_full_run{run_id:03d}_rate{rate:03d}.png"
...
def save_residual_cyclewise_plot(pred: pd.DataFrame, run_id: int, rate: int) -> None:
    df = pred[
        (pred["run_id"].astype(int) == int(run_id))
        & (pred["cv_rate_pct"].astype(int) == int(rate))
    ].copy()
    ...
    for i, (start, end) in enumerate(cycles):
        ...
        if BASELINE_Q_COL in g.columns:
            ax.plot(
                g["time_sec"],
                pd.to_numeric(g[BASELINE_Q_COL], errors="coerce"),
                color="0.45",
                linewidth=1.15,
                linestyle="--",
                label="Cumulative Count Theory",
    ...
    fig.suptitle(f"Residual sequence diagnostic cycles | Run {run_id:03d} | CV {rate}%", y=0.995)
    fig.supxlabel("Time (s)")
    fig.supylabel("Queue length (ft)")
    fig.tight_layout(rect=(0, 0, 1, 0.96))

    out_path = FIG_DIR / f"residual_sequence_cyclewise_run{run_id:03d}_rate{rate:03d}.png"
    fig.savefig(out_path, dpi=300)
    plt.close(fig)
    print(f"[Saved diagnostic figure] {out_path}")


def save_residual_diagnostic_plots(pred: pd.DataFrame) -> None:
    ...
    for run_id in run_ids:
        save_residual_full_run_plot(pred, int(run_id), int(DIAGNOSTIC_CV_RATE_PCT))
        save_residual_cyclewise_plot(pred, int(run_id), int(DIAGNOSTIC_CV_RATE_PCT))
...
def main() -> None:
    ...
    print("=" * 96)
    print("Residual sequence diagnostic training")
    print("=" * 96)
    ...
    pred_all_path = OUT_DIR / "residual_sequence_predictions_allruns_allrates.csv"
    pred.to_csv(pred_all_path, index=False)
    print(f"\n[Saved combined predictions] {pred_all_path}")

    if SAVE_PER_RUN_RATE_FILES:
        for (run_id, rate), g in pred.groupby(["run_id", "cv_rate_pct"], sort=True):
            out_path = OUT_DIR / f"residual_sequence_predictions_run{int(run_id):03d}_rate{format_rate(int(rate))}.csv"
    ...
    if not summary.empty:
        ...
        summary.to_csv(OUT_DIR / "residual_sequence_training_summary.csv", index=False)
        print(f"[Saved] {OUT_DIR / 'residual_sequence_training_summary.csv'}")

    if history_parts:
        history = pd.concat(history_parts, ignore_index=True)
        history.to_csv(OUT_DIR / "nn_training_history_residual_sequence.csv", index=False)
        print(f"[Saved] {OUT_DIR / 'nn_training_history_residual_sequence.csv'}")

    if feature_parts:
        features_used = pd.concat(feature_parts, ignore_index=True)
        features_used.to_csv(OUT_DIR / "feature_columns_used_residual_sequence.csv", index=False)
        print(f"[Saved] {OUT_DIR / 'feature_columns_used_residual_sequence.csv'}")

    metrics_rate = compute_metrics_by_run_rate(pred)
    metrics_run = summarize_metrics_by_run(metrics_rate)

    metrics_rate_path = OUT_DIR / "residual_sequence_metrics_by_model_run_rate.csv"
    metrics_run_path = OUT_DIR / "residual_sequence_metrics_by_model_run.csv"
    ...
    save_residual_diagnostic_plots(pred)
